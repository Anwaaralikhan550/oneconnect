...
class GymDetailScreen extends StatefulWidget {
  ...
  const GymDetailScreen({super.key, this.gymData});
...
class _GymDetailScreenState extends State<GymDetailScreen>
  ...
  Widget build(BuildContext context) {
    ...
    final detail = provider.getAmenityDetail(widget.gymData?['id'] ?? '');
    final phone = (detail?.phone ?? widget.gymData?['phone']?.toString() ?? '').trim();
    final whatsapp =
        (detail?.whatsapp ?? widget.gymData?['whatsapp']?.toString() ?? phone).trim();
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          DetailScreenHeader(
            title: 'Gym',
            iconAssetPath: 'assets/images/gym_icon_hub.svg',
            fallbackIcon: Icons.fitness_center,
            onShare: () {
              final name = detail?.name ?? widget.gymData?['name'] ?? 'Gym';
            ...
            child: SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  LocationSection(
              locationText: detail?.location ?? widget.gymData?['location'] ?? '',
            ),
                  ...
                  FacilitiesSection(
                    items: const [
                      FacilityItem(icon: Icons.fitness_center, label: 'Weights'),
                      FacilityItem(icon: Icons.directions_run, label: 'Cardio'),
                      FacilityItem(icon: Icons.pool, label: 'Swimming'),
                      FacilityItem(icon: Icons.hot_tub, label: 'Sauna'),
                      FacilityItem(icon: Icons.shower, label: 'Showers'),
                      FacilityItem(icon: Icons.local_parking, label: 'Parking'),
                      FacilityItem(icon: Icons.wifi, label: 'Free WiFi'),
                      FacilityItem(icon: Icons.lock, label: 'Lockers'),
                    ],
                    notice: Container(
                      padding: EdgeInsets.all(rw(10)),
                      decoration: BoxDecoration(color: const Color(0xFFE8F5E9), borderRadius: BorderRadius.circular(10)),
                      child: Text('Free trial session for new members', style: GoogleFonts.inter(fontSize: rfs(11), fontWeight: FontWeight.w500, color: const Color(0xFF2E7D32), height: 16 / 11), textAlign: TextAlign.center),
                    ),
                  ),
                  SizedBox(height: rh(10)),
              ...
              onGiveReview: () {
                GiveReviewDialog.show(
                  context: context,
                  itemName: detail?.name ?? widget.gymData?['name'] ?? 'Gym',
                  itemLocation: detail?.location ?? widget.gymData?['location'] ?? '',
                  itemRating: detail?.rating ?? double.tryParse(widget.gymData?['rating']?.toString() ?? '0') ?? 0.0,
                  reviewCount: detail?.reviewCount ?? int.tryParse(widget.gymData?['reviewCount']?.toString() ?? widget.gymData?['reviews']?.toString() ?? '0') ?? 0,
                  itemImage: detail?.imageUrl ?? widget.gymData?['image'],
                  itemTypeHint: 'GYM',
                  onSubmit: (rating, review, hasPhoto, hasVideo) async {
                    final entityId = widget.gymData?['id'] as String?;
                    if (entityId == null) {
                      if (context.mounted) {
        ...
        children: [
          ClipOval(
            child: SizedBox(
              width: rw(90),
              height: rw(90),
              child: buildProfileImage(detail?.imageUrl ?? widget.gymData?['image'], fallbackIcon: Icons.fitness_center, iconSize: 50),
            ),
          ),
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            detail?.name ?? widget.gymData?['name'] ?? 'Gym',
                          ...
                          Text(
                            detail?.location ?? widget.gymData?['location'] ?? '',
                    ...
                    Consumer<FavoriteProvider>(
                      builder: (context, favProvider, _) {
                        final amenityId = widget.gymData?['id'] as String?;
                  ...
                  children: [
                    Row(
                      children: List.generate(5, (index) {
                        final ratingVal = detail?.rating ?? double.tryParse(widget.gymData?['rating']?.toString() ?? '') ?? 4.8;
                        return Icon(
                    ...
                    SizedBox(width: rw(8)),
                    Text(
                      detail != null ? detail.rating.toStringAsFixed(1) : (widget.gymData?['rating'] ?? '4.8'),
                      style: GoogleFonts.inter(fontSize: rfs(12), fontWeight: FontWeight.w500, color: const Color(0xFF353535), letterSpacing: 0.168),
                    ),
                    SizedBox(width: rw(4)),
                    Flexible(
                      child: Text(
                        '(${detail != null ? detail.reviewCount : (widget.gymData?['reviewCount'] ?? widget.gymData?['reviews'] ?? '320')} Reviews)',
                        style: GoogleFonts.inter(fontSize: rfs(12), fontWeight: FontWeight.w500, color: const Color(0xFF353535), letterSpacing: 0.168),
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                      ),
  ...
  Widget _buildTimingsSection() {
    final detail = Provider.of<BusinessProvider>(context, listen: false)
        .getAmenityDetail(widget.gymData?['id'] ?? '');
    final opening = detail?.openingTime?.toString() ??
        widget.gymData?['openingTime']?.toString();
    final closing = detail?.closingTime?.toString() ??
        widget.gymData?['closingTime']?.toString();
    ...
    final days = detail?.operatingDays ??
        ((widget.gymData?['operatingDays'] as List?)
                ?.map((e) => e.toString())
                .toList() ??
            const <String>[]);
    final daysText = days.isNotEmpty ? days.join(', ') : 'Monday - Saturday';
    final distanceText = widget.gymData?['distance']?.toString() ?? '1.5 Km away';
            ...
            child: Row(
              ...
              children: [
                SvgPicture.asset(
                  ...
                  width: rw(25),
                  height: rw(25),
                  colorFilter: const ColorFilter.mode(Color(0xFF0097B2), BlendMode.srcIn),
                  placeholderBuilder: (context) => Icon(Icons.check_circle, size: rw(25), color: const Color(0xFF0097B2)),
                ),
                SizedBox(width: rw(5)),
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        'Opening Hours',
                        style: GoogleFonts.roboto(fontSize: rfs(15), fontWeight: FontWeight.w500, color: Colors.black),
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                      ),
                      ...
                      Text(
                        timeText,
                        style: GoogleFonts.roboto(fontSize: rfs(13), fontWeight: FontWeight.w400, color: Colors.black),
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                      ),
                      SizedBox(height: rh(2)),
                      Text(
                        daysText,
                        style: GoogleFonts.roboto(fontSize: rfs(12), fontWeight: FontWeight.w400, color: const Color(0xFF727272)),
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                      ),
                    ],
          ...
          Row(
            ...
            children: [
              SvgPicture.asset(
                ...
                width: rw(25),
                height: rw(24.943),
                placeholderBuilder: (context) => Icon(Icons.directions_walk, size: rw(25), color: const Color(0xFF0097B2)),
              ),
              SizedBox(width: rw(5)),
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  ...
                  Text(
                    distanceText,
                    style: GoogleFonts.roboto(fontSize: rfs(13), fontWeight: FontWeight.w400, color: const Color(0xFF202020)),
                  ),
                ],
              ),
            ],
          ...
          SizedBox(
            ...
            child: Column(
              ...
              children: [
                Text(
                  'Membership',
                  style: GoogleFonts.inter(fontSize: rfs(10), fontWeight: FontWeight.w700, color: Colors.black, height: 16 / 10),
                ),
                SizedBox(height: rh(10)),
                Row(
                  children: [
                    Icon(Icons.card_membership, size: rw(30), color: Colors.black),
                    SizedBox(width: rw(5)),
                    Flexible(
                      child: Text(
                        'Rs. 5,500/mo',
                        style: GoogleFonts.inter(fontSize: rfs(18), fontWeight: FontWeight.w700, color: Colors.black),
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                      ),
                    ),
          ...
          Container(
            ...
            child: Center(
              child: Text(
                ...
                style: GoogleFonts.inter(fontSize: rfs(11), fontWeight: FontWeight.w500, color: const Color(0xFF1565C0), height: 16 / 11),
