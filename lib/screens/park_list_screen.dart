...
class _ParkListScreenState extends State<ParkListScreen>
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ListScreenHeader(
              title: 'Parks',
              categoryIconAsset: 'assets/images/park_icon_hub.svg',
              onSearch: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const SearchScreen())),
            ),
          Expanded(
            child: Consumer<BusinessProvider>(
              builder: (context, provider, child) {
                final amenities = provider.getAmenities('PARK');
  ...
  Widget _buildTopParkSection(AmenityModel park) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Padding(
          ...
          child: Row(
            children: [
              ...
              Flexible(child: Text('Top Park of the month', style: GoogleFonts.inter(fontSize: rfs(18), fontWeight: FontWeight.w700, color: Colors.black, letterSpacing: 0.112, height: 1.2), overflow: TextOverflow.ellipsis)),
            ],
          ),
        ),
        ...
        Center(
          child: GestureDetector(
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => ParkDetailScreen(parkData: {
              'id': park.id,
              'name': park.name,
              'rating': park.rating.toStringAsFixed(1),
              'reviews': park.reviewCount.toString(),
                    ...
                    child: Stack(
                      children: [
                        ClipRRect(
                          ...
                          child: SizedBox(width: rw(343), height: rh(135), child: buildProfileImage(park.imageUrl, fallbackIcon: Icons.park, iconSize: rw(50))),
                        ),
                        Positioned(top: rh(6), left: rw(5), child: Container(padding: EdgeInsets.symmetric(horizontal: rw(5), vertical: rh(5)), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(rw(14))), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.star, size: rw(20), color: const Color(0xFFFFCD29)), SizedBox(width: rw(2)), Text(park.rating.toStringAsFixed(1), style: GoogleFonts.roboto(fontSize: rfs(12), fontWeight: FontWeight.w500, color: Colors.black)), SizedBox(width: rw(2)), Text('(${park.reviewCount})', style: GoogleFonts.inter(fontSize: rfs(10), fontWeight: FontWeight.w500, color: Colors.black))]))),
                    ...
                    child: Row(
                      ...
                      children: [
                        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text(park.name, style: GoogleFonts.oswald(fontSize: rfs(16), fontWeight: FontWeight.w400, color: Colors.black)),
                          Row(children: [SvgPicture.asset('assets/icons/figma_location_icon.svg', width: rw(17), height: rw(17), placeholderBuilder: (context) => Icon(Icons.location_on, size: rw(17), color: Colors.grey)), Text(park.location ?? '', style: GoogleFonts.oswald(fontSize: rfs(11), fontWeight: FontWeight.w400, color: Colors.black))]),
                        ]),
                        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Row(mainAxisSize: MainAxisSize.min, children: [SvgPicture.asset('assets/icons/figma_open_door_icon.svg', width: rw(24), height: rw(24), placeholderBuilder: (context) => Icon(Icons.door_front_door, size: rw(24), color: const Color(0xFF073A6A))), Text(park.isOpen ? 'Open' : 'Closed', style: GoogleFonts.oswald(fontSize: rfs(11), fontWeight: FontWeight.w400, color: park.isOpen ? const Color(0xFF073A6A) : Colors.red))]),
  ...
  Widget _buildFrequentlyVisitedCard(AmenityModel park) {
        ...
        child: Column(
          children: [
            SizedBox(
              ...
              child: Stack(
                children: [
                  ClipRRect(borderRadius: BorderRadius.circular(rw(20)), child: SizedBox(width: rw(302), height: rh(118), child: buildProfileImage(park.imageUrl, fallbackIcon: Icons.park, iconSize: rw(40)))),
            ...
            Container(
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(park.name, style: GoogleFonts.oswald(fontSize: rfs(15), fontWeight: FontWeight.w400, color: Colors.black)), Row(children: [SvgPicture.asset('assets/icons/figma_location_icon.svg', width: rw(17), height: rw(17), placeholderBuilder: (context) => Icon(Icons.location_on, size: rw(17), color: Colors.grey)), Text(park.location ?? '', style: GoogleFonts.oswald(fontSize: rfs(11), fontWeight: FontWeight.w300, color: Colors.black))])]),
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(mainAxisSize: MainAxisSize.min, children: [SvgPicture.asset('assets/icons/figma_open_door_icon.svg', width: rw(24), height: rw(24), placeholderBuilder: (context) => Icon(Icons.door_front_door, size: rw(24), color: const Color(0xFF073A6A))), Text(park.isOpen ? 'Open' : 'Closed', style: GoogleFonts.oswald(fontSize: rfs(11), fontWeight: FontWeight.w400, color: park.isOpen ? const Color(0xFF073A6A) : Colors.red))]), Row(mainAxisSize: MainAxisSize.min, children: [SvgPicture.asset('assets/icons/figma_map_icon.svg', width: rw(24), height: rw(24), placeholderBuilder: (context) => Icon(Icons.map, size: rw(24), color: const Color(0xFF6A6A6A))), SizedBox(width: rw(1)), Text('Map', style: GoogleFonts.oswald(fontSize: rfs(11), fontWeight: FontWeight.w400, color: const Color(0xFF073A6A)))])]),
                ],
              ),
            ),
          ],
  ...
  Widget _buildParksNearLocationSection(List<AmenityModel> items) {
    if (items.isEmpty) return const SizedBox.shrink();
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Padding(
          padding: EdgeInsets.symmetric(horizontal: rw(20)),
          child: Row(children: [SvgPicture.asset('assets/icons/figma_location_filled_icon.svg', width: rw(25), height: rw(25), colorFilter: const ColorFilter.mode(Color(0xFF0097B2), BlendMode.srcIn), placeholderBuilder: (context) => Icon(Icons.location_on, size: rw(25), color: const Color(0xFF0097B2))), SizedBox(width: rw(15)), Text('Parks near your location', style: GoogleFonts.inter(fontSize: rfs(18), fontWeight: FontWeight.w700, color: Colors.black, letterSpacing: 0.112, height: 1.2))]),
        ),
        SizedBox(height: rh(20)),
        Padding(padding: EdgeInsets.symmetric(horizontal: rw(15)), child: Column(children: items.map((park) => Padding(padding: EdgeInsets.only(bottom: rh(20)), child: _buildNearbyParkCard(park))).toList())),
      ],
    );
  }

  Widget _buildNearbyParkCard(AmenityModel park) {
    return GestureDetector(
      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => ParkDetailScreen(parkData: {
        'id': park.id,
        'name': park.name,
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ClipRRect(borderRadius: BorderRadius.circular(rw(100)), child: SizedBox(width: rw(81), height: rw(81), child: buildProfileImage(park.imageUrl, fallbackIcon: Icons.park, iconSize: rw(30)))),
            SizedBox(width: rw(10)),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text(park.name, style: GoogleFonts.inter(fontSize: rfs(15), fontWeight: FontWeight.w600, color: Colors.black), maxLines: 1, overflow: TextOverflow.ellipsis),
                  Row(children: [SvgPicture.asset('assets/icons/figma_location_icon.svg', width: rw(17), height: rw(17), placeholderBuilder: (context) => Icon(Icons.location_on, size: rw(17), color: Colors.grey)), Text(park.location ?? '', style: GoogleFonts.oswald(fontSize: rfs(11), fontWeight: FontWeight.w300, color: Colors.black))]),
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: List.generate(5, (index) => Icon(index < park.rating.round() ? Icons.star : Icons.star_border, size: rw(15), color: const Color(0xFFFFCD29)))), SizedBox(height: rh(4)), Row(children: [Text(park.rating.toStringAsFixed(1), style: GoogleFonts.inter(fontSize: rfs(12), fontWeight: FontWeight.w500, color: const Color(0xFF353535), letterSpacing: 0.168)), SizedBox(width: rw(3)), Text('(${park.reviewCount} Reviews)', style: GoogleFonts.inter(fontSize: rfs(12), fontWeight: FontWeight.w500, color: const Color(0xFF353535), letterSpacing: 0.168))])]),
                ],
              ),
            ),
